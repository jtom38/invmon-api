...
class HtmlParser:
    ...
    Examples:
    HtmlParser(url='www').
    HtmlParser().setUrl("www").
    """

    _logger: LoggerInterface

    def __init__(self, url: str = "", sourceCode: str = '') -> None:
        self.url = url
        if sourceCode != '':
            self.setSource(sourceCode)
            self.generateSoup()

    def setLogger(self, logger: LoggerInterface) -> None:
        self._logger = logger

    def setSource(self, htmlSource: str) -> None:
        self.__source__ = htmlSource
    ...
    def setSoup(self, soup: BeautifulSoup) -> None:
        ...
        if self.__soup__ == None:
            self.__soup__ = self.__genSoup__()
        self.soup = soup
        self.__soup__ = soup
    ...
    def __genSoup__(self) -> BeautifulSoup:
        try:
            soup = BeautifulSoup(self.__source__, features="html.parser")
            return soup
        except Exception as e:
    ...
    def __validateSoup__(self) -> bool:
        if self.__soup__ == None:
            return False
        return True
    ...
    def findSingle(self, name: str = "", attrKey: str = "", attrValue: str = "") -> BeautifulSoup:
        if self.__validateSoup__ == False:
            raise Exception("Missing soup object to parse.  use generateSoup() to build the object.")

        if attrKey != "":
            attrs = {attrKey: attrValue}
            #res =self.__soup__.find_all(name=name, arrts=attrs)
...
class RequestSiteContent(HtmlParser):
...
class RequestArticleContent(HtmlParser):
