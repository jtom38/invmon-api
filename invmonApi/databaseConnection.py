...
class ConnectionString():
    ...
    Env Values:
        INVMON_DATABASE_NAME = 'newsbot'
        INVMON_DATABASE_TYPE = 'sqlite'
    ...
    def __init__(self) -> None:
        ...
        if self.__dbType__ == ConnectionStringTypes.POSTGRES:
            ...
            self.__dbUsername__ = str(getenv("INVMON_DATABASE_USERNAME"))
            self.__dbPassword__ = str(getenv("INVMON_DATABASE_PASSWORD"))
            self.__dbHost__ = str(getenv("INVMON_DATABASE_HOST"))
            pass
    ...
    def readEnvName(self) -> str:
        name = str(getenv("INVMON_DATABASE_NAME")).lower()
        if name == "none":
    ...
    def readEnvMode(self) -> str:
        mode = str(getenv("INVMON_ENV")).lower()
        if mode == 'none':
    ...
    def readEnvType(self) -> ConnectionStringTypes:
        res = str(getenv("INVMON_DATABASE_TYPE")).lower()
        if res == "sqlite":
    ...
    def readEnvDbPath(self) -> str:
        res = str(getenv("INVMON_SQLITE_PATH"))
        if res == "None":
